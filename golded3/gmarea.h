...
public:
    ...
    {
        if(value) marks |= (word)(1<<AL.mask);
        else marks &= (word)(~(1<<AL.mask));
    }
    ...
    {
        adat->msglistdate++;
        if(adat->msglistdate > MSGLISTDATE_RECEIVED) adat->msglistdate = MSGLISTDATE_NONE;
        return adat->msglistdate;
    }
