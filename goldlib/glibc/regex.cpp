...
static bool at_begline_loc_p(const char *pattern, const char *p,
                             reg_syntax_t syntax);
static bool at_endline_loc_p(const char *p, const char *pend,
                             reg_syntax_t syntax);
...
static bool group_in_compile_stack(compile_stack_type compile_stack,
                                   regnum_t regnum);
...
static bool alt_match_null_string_p(unsigned char *p, unsigned char *end,
                                    register_info_type *reg_info);
static bool common_op_match_null_string_p(unsigned char **p,
        unsigned char *end, register_info_type *reg_info);
    ...
    for (;;)
    {
        if (p == pend)
        {
            ...
            if (d != end_match_2)
            {
                ...
                if ((FIRST_STRING_P (match_end) == MATCHING_IN_FIRST_STRING))
...
succeed_label:
        ...
        case charset_not:
        {
            ...
            bool not_bool = (re_opcode_t) *(p - 1) == charset_not;
            PREFETCH ();
